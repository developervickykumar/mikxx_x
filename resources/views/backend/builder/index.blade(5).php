    ...
    return `
        <div class="checkbox-table-wrapper mb-3" id="tableWrapper-${label}">
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <div>
                    <button class="btn btn-sm btn-warning" onclick="duplicateCheckboxTable('${label}')">Duplicate</button>
                    <button class="btn btn-sm btn-danger" onclick="removeCheckboxTable('${label}')">Remove</button>
                </div>
            </div>
            ...
            <table class="table table-bordered" id="checkboxTable-${label}">
...
function addRowToCheckboxTable(label) {
    ...
    row.innerHTML = `
        <th contenteditable="true">${rowLabel}</th>
        ${'<td><input type="checkbox" /></td>'.repeat(colCount)}
...
function normalizeTableRows(label) {
    ...
    const theadRow = table.querySelector('thead tr');
    const headerCount = theadRow ? theadRow.querySelectorAll('th').length - 1 : 0;

    const tbodyRows = table.querySelectorAll('tbody tr');
    tbodyRows.forEach(row => {
        const cells = row.querySelectorAll('td');
        const checkboxCells = Array.from(cells).filter(td => td.querySelector('input[type="checkbox"]'));
        const currentCheckboxCount = checkboxCells.length;

        // Remove existing checkboxes + remove button
        checkboxCells.forEach(td => td.remove());
        const removeBtn = row.querySelector('button');
        if (removeBtn) removeBtn.closest('td').remove();

        // Add proper number of checkboxes
